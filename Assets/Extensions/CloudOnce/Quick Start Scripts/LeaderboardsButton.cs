...
namespace CloudOnce.QuickStart
{
    ...
    public class LeaderboardsButton : MonoBehaviour
    {
        ...
        private void OnButtonClicked()
        {
            ...
            {
                Cloud.Leaderboards.ShowOverlay();
                GameObject.Find("Debug").GetComponent<TextMeshProUGUI>().text="Muestra Algo";
            }
            else
            {
#if CLOUDONCE_DEBUG
                Debug.Log("Can't show leaderboard overlay, user is not signed in!");
                GameObject.Find("Debug").GetComponent<TextMeshProUGUI>().text="Can't show leaderboard overlay, user is not signed in!";
#endif
                SubscribeEvent();
                Cloud.SignIn();
            }
